 ...
 *
 * Modifications by: Evandro Copercini ( https://github.com/copercini )
 * Remove unnecessary headers
 * Optimize buffers size
 * Reduce stack overflow possibility 
 * Fix rmdir for Arduino
 *
 ...
 * do not contain '/spiffs' prefix or './' sufix
 */
...
static int IRAM_ATTR vfs_spiffs_unlink(const char *path)
{
    ...
    if (is_dir(path))
    {
        ...
        int nument = 0;
        char *npath2 = NULL;
        npath2 = path_join("/spiffs", path);

        DIR *dir = opendir(npath2);
        free(npath2);
        ...
        if (nument > 0)
        {
            log_e("Directory not empty, cannot remove");
            errno = ENOTEMPTY;
            return -1;
        }
    ...
    spiffs_file FP;
    if (npath != NULL)
        FP = SPIFFS_open(&fs, npath, SPIFFS_RDWR, 0);
    else
        FP = SPIFFS_open(&fs, path, SPIFFS_RDWR, 0);
...
int spiffs_mount()
{
    ...
    log_i("Mounting SPIFFS files system");
    ...
    log_i("Start address: 0x%x; Size %d KB", cfg.phys_addr, cfg.phys_size / 1024);
    log_i("  Work buffer: %d B", cfg.log_page_size * 8);
    log_i("   FDS buffer: %d B", sizeof(spiffs_fd) * SPIFFS_TEMPORAL_CACHE_HIT_SCORE);
    log_i("   Cache size: %d B", cfg.log_page_size * SPIFFS_TEMPORAL_CACHE_HIT_SCORE);
    while (retries < 2)
    {
        res = SPIFFS_mount(
        ...
        if (res < 0)
        {
            ...
            {
                log_w("No file system detected, formating...");
                SPIFFS_unmount(&fs);
...
err_exit:
    log_e("Error allocating fs structures (%d)", err);
exit:
...
int vfs_spiffs_register(const char *mountpoint)
{
    ...
    log_i("Registering SPIFFS file system");
    esp_err_t res = esp_vfs_register(mountpoint, &vfs, NULL);
    if (res != ESP_OK)
    {
